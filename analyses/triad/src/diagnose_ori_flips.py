...
def correct_mounting_ori(df, action_col='mounting_attempt',
                          within_bout_jump_threshold_rad=1.0, inplace=False):
    """
    Correct orientation for mounting-attempt frames in two passes.

    Pass 1 — per-frame abs_ang_between comparison:
        For each mounting frame, choose ori or ori+π based on which points
        more toward the nearest other fly (by dist_to_other / abs_ang_between).

    Pass 2 — within-bout consistency smoothing:
        Within each contiguous mounting bout, scan forward and at any jump
        exceeding within_bout_jump_threshold_rad, flip the current frame and
        all subsequent bout frames by π — but only if flipping actually reduces
        the jump. This corrects cases where the fly is temporarily oblique to
        the target, causing pass 1 to assign an inconsistent orientation.

    The action_col convention (same as all other actions): value = actor fly's
    id when mounting is active, -1 otherwise. Only actor-fly rows carry a
    non-(-1) value, so filtering action_col != -1 gives actor-only rows.

    The write-back updates all pair rows for each (id, frame) key.

    Arguments:
        df         -- tracks df with pairwise metrics already computed
                      (needs 'dist_to_other', 'abs_ang_between', action_col)

    Keyword Arguments:
        action_col                   -- column whose non-(-1) value is the
                                        actor fly's id (default: 'mounting_attempt')
        within_bout_jump_threshold_rad -- max tolerated jump within a bout before
                                          triggering a flip (default: 1.0 rad)
        inplace                      -- modify df in place (default: False)

    Returns:
        df with ori corrected for mounting frames
    """
    if not inplace:
        df = df.copy()
    ...
    for col in (action_col, 'dist_to_other', 'abs_ang_between'):
        if col not in df.columns:
            print(f"No '{col}' column — skipping mounting orientation correction.")
            return df

    # action_col != -1 gives actor-only rows (actor fly id is the stored value)
    mount_rows = df[df[action_col] != -1]
    if len(mount_rows) == 0:
        print(f"No mounting frames found in '{action_col}' — skipping.")
        return df

    # ── Pass 1: per-frame comparison to abs_ang_between ──────────────────────
    # In a triad the actor has one pair row per other fly; pick the nearest
    # to get the most relevant abs_ang_between.
    nearest = mount_rows.loc[
        mount_rows.groupby(['frame', 'id'])['dist_to_other'].idxmin()
    ][['frame', 'id', 'ori', 'abs_ang_between']].dropna()

    n_pass1 = 0
    for _, row in nearest.iterrows():
        ori         = float(row['ori'])
        target_ang  = float(row['abs_ang_between'])
        ori_flipped = float(np.arctan2(-np.sin(ori), -np.cos(ori)))

        if (abs(_circ_diff(ori_flipped, target_ang)) <
                abs(_circ_diff(ori, target_ang))):
            mask = (df['id'] == row['id']) & (df['frame'] == row['frame'])
            df.loc[mask, 'ori'] = ori_flipped
            n_pass1 += 1

    # ── Pass 2: within-bout jump smoothing (commented out for now) ───────────
    # n_pass2 = 0
    # fly_ids = mount_rows['id'].unique()
    # for fly_id in fly_ids:
    #     fly_row_mask = df['id'] == fly_id
    #     fly_df = (df[fly_row_mask]
    #               .drop_duplicates('frame')
    #               .sort_values('frame')
    #               .reset_index(drop=True))
    #     mount_frames = sorted(
    #         df.loc[fly_row_mask & (df[action_col] != -1), 'frame'].unique()
    #     )
    #     if not mount_frames:
    #         continue
    #     bouts, bout_start, prev = [], mount_frames[0], mount_frames[0]
    #     for f in mount_frames[1:]:
    #         if f - prev > 1:
    ...
    #         ctx_mask = (fly_df['frame'] >= b_start - 1) & (fly_df['frame'] <= b_end)
    #         bout_frames = fly_df.loc[ctx_mask, 'frame'].values
    #         ori_bout    = fly_df.loc[ctx_mask, 'ori'].values.astype(float)
    #         for i in range(1, len(ori_bout)):
    #             if np.isnan(ori_bout[i]) or np.isnan(ori_bout[i - 1]):
    #                 continue
    #             diff = abs(_circ_diff(ori_bout[i], ori_bout[i - 1]))
    #             if diff > within_bout_jump_threshold_rad:
    #                 candidate    = np.arctan2(-np.sin(ori_bout[i:]), -np.cos(ori_bout[i:]))
    #                 diff_flipped = abs(_circ_diff(candidate[0], ori_bout[i - 1]))
    #                 if diff_flipped < diff:
    #                     ori_bout[i:] = candidate
    #         for frame_val, ori_val in zip(bout_frames, ori_bout):
    #             if frame_val < b_start:
    #                 continue
    #             mask = fly_row_mask & (df['frame'] == frame_val)
    #             old  = df.loc[mask, 'ori'].values
    #             if len(old) and not np.isclose(float(old[0]), ori_val, atol=1e-6):
    #                 df.loc[mask, 'ori'] = ori_val
    #                 n_pass2 += 1

    n_pass2 = 0
    print(f"Mounting correction: pass1 flipped {n_pass1}, "
          f"pass2 smoothed {n_pass2} frames.")
    return df
